...
Outputs written to _data/:
  ...
  refdata_meta.json                 high-level structure of the TTC
  refdata_pua_glyphs.json           every PUA glyph with metrics (advance,
                                   LSB, bbox, numContours)
  refdata_cjk_composites.json       for each sub-font, CJK char ->
                                   composite components and offsets
  refdata_pua_syllable_map.json     PUA codepoint -> pinyin syllable
                                   (derived via pypinyin for chars that
                                   reference the PUA in sub-font 0)
  refdata_pua_syllable_conflicts.json  diagnostic: PUAs where chars
...
Run: python scripts/extract_refdata.py
"""
...
def main() -> int:
    ...
    meta = {
        ...
        "extractor": "scripts/extract_refdata.py",
        "n_sub_fonts": n_sub,
        "sub_fonts": [],
    }
    ...
    # --- write outputs ----------------------------------------------------
    outputs = {
        "refdata_meta.json": meta,
        "refdata_pua_glyphs.json": {"upm": meta["sub_fonts"][0]["unitsPerEm"], "glyphs": pua_glyphs},
        "refdata_cjk_composites.json": cjk_composites,
        "refdata_pua_syllable_map.json": pua_syllable_map,
        "refdata_pua_syllable_conflicts.json": conflicts,
    }
    print()
    print("writing files:")
